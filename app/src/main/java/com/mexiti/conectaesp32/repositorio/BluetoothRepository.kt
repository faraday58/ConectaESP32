...
class BluetoothRepository(private val context:Context){
    ...
    fun connectToDevice(deviceAddress: String, scope: CoroutineScope){
        ...
        val device: BluetoothDevice? = try {
            ...
            _connectionState.value = ConnectionState.Error("Dirección MAC inválida " + e.message )
            null
        }
    ...
    private fun startReceivingData(coroutineScope: CoroutineScope) {
        ...
        jobLectura = coroutineScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                // Ejecutar limpieza solo cuando el bucle termine
                if (isActive && _connectionState.value is ConnectionState.Connected) {
                    Log.d(TAG, "Bucle de lectura terminado - desconectando")
                    _connectionState.value = ConnectionState.Disconnected
                }
